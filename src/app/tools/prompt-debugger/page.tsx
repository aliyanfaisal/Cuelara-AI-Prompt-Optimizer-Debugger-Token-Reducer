...
export default function PromptDebuggerPage() {
  ...
  const handleFixNow = (issue: DebuggerIssue) => {
    setInput((prev) => {
      const trimmed = prev.trimEnd();
      if (trimmed.includes(issue.fix)) return prev;
                  ...
                  <div>
                    <span className="font-bold text-sm text-foreground">
                      {issues.length === 0
                        ...
                        : `${issues.length} Vulnerabilit${issues.length === 1 ? "y" : "ies"} Detected`}
                    </span>
                    ...
                    >
                      ...
                      <div className="bg-background border border-border rounded-lg p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button
                            onClick={() => handleFixNow(issue)}
                            disabled={isApplied}
                            className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-600 hover:bg-emerald-700 text-[11px] font-semibold text-white transition-colors disabled:opacity-60 disabled:cursor-default"
                          >
                            {isApplied ? (
                              <>
              ...
              {appliedFixIds.size > 0 && (
                <div className="px-5 md:px-6 pb-5 md:pb-6">
                  <p className="text-xs text-muted-foreground italic">
                    Applied fixes were appended to your prompt above — re-run the audit anytime to verify.
                  </p>
                </div>
              )}
            </motion.div>
