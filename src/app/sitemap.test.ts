...
describe("sitemap.xml", () => {
  it("lists the static public pages, and not admin, auth or noindex placeholder pages", async () => {
    ...
    for (const path of ["/", "/tools", "/tools/prompt-optimizer", "/blog", "/cookbook", "/pricing", "/contact", "/about", "/privacy", "/terms"]) assert.ok(urls.includes(`${BASE}${path}`), path);
    for (const path of ["/admin", "/login", "/docs", "/changelog"]) assert.ok(!urls.some((u) => u.startsWith(`${BASE}${path}`)), path);
  });
