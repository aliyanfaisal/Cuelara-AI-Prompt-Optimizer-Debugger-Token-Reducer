...
export const designDnaSchema = z.object({
  ...
  typography: z.object({
    ...
    scale: z
      .array(z.object({ role: short, size: z.number(), weight: z.number(), lineHeight: z.number().nullable() }))
      .max(10),
  }),
  ...
  shadows: z.array(z.string().max(200)).max(4),
  effects: z.object({ backdropBlur: z.boolean(), gradients: z.array(z.string().max(300)).max(4) }),
  layout: z.object({
  ...
  components: z.object({
    button: z
      .object({ bg: hex, color: hex, radius: z.number(), padding: short, fontWeight: z.number(), shadow: z.string().max(200).nullable() })
      .nullable(),
    card: z
      .object({ bg: hex, color: hex, radius: z.number(), padding: short, fontWeight: z.number(), shadow: z.string().max(200).nullable() })
      .nullable(),
...
const rawSampleSchema = z.object({
  ...
  color: cssColor,
  bg: cssColor,
  bgImage: z.string().max(300),
  fontFamily: z.string().max(200),
  fontSize: num,
  fontWeight: num,
  lineHeight: num,
  radius: num,
  shadow: z.string().max(200),
  border: num,
