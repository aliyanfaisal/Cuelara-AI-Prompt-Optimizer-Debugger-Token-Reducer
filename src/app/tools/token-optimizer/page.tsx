...
const LOADING_PHRASES = [
  ...
  "Compressing structure without losing intent...",
  "Finalizing token savings..."
];
    ...
    <div className="flex flex-col w-full max-w-4xl mx-auto px-6 py-8">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground">
          Compress your prompt token count by up to 50% without losing meaning, constraints, or functionality.
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="px-5 py-3 border-b border-border/40 bg-muted/20 flex flex-wrap items-center gap-4">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border bg-background text-xs font-medium hover:bg-muted hover:border-foreground/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              Preserve Format: <span className="text-foreground">{preserve}</span>
              <ChevronDown className="w-3.5 h-3.5 opacity-50" />
            </button>
            {isPreserveOpen && (
        ...
        <div className="relative p-5">
          <textarea
            ...
            placeholder="Paste your bloated prompt here to squish those tokens..."
        ...
        <div className="px-5 py-4 border-t border-border/40 bg-muted/10 flex items-center justify-between">
          <div className="text-[11px] text-muted-foreground flex items-center gap-1.5">
            ...
            Current est. {Math.max(0, Math.floor(input.length / 4))} tokens
          </div>

          <button
            onClick={handleCompress}
            disabled={!input.trim() || state === "loading"}
            className="relative overflow-hidden group flex items-center gap-2 px-6 py-2.5 rounded-xl bg-amber-500 text-white text-sm font-bold shadow-[0_0_20px_rgba(245,158,11,0.2)] hover:shadow-[0_0_25px_rgba(245,158,11,0.3)] transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-[0_0_20px_rgba(245,158,11,0.2)]"
          >
            {state === "loading" ? (
              <>
        ...
        <AnimatePresence mode="wait">
          ...
          {state === "success" && (
            <motion.div
              ...
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-card border border-amber-500/20 rounded-2xl overflow-hidden shadow-[0_4px_30px_rgba(245,158,11,0.05)] mt-4"
            >
              {/* Savings Banner */}
              <div className="px-5 py-4 border-b border-border/40 bg-amber-500/5 flex items-center justify-between">
                <div className="flex items-center gap-3 text-sm font-bold text-foreground">
                  <div className="p-1.5 bg-amber-500/20 text-amber-500 rounded-md">
                    <Zap className="w-4 h-4" />
                  </div>
                  <div>
                    <span className="text-emerald-500">-42% Tokens</span>
                    <span className="text-muted-foreground ml-2 text-xs font-medium">
                      ({Math.floor(input.length / 4)} → {Math.floor((input.length / 4) * 0.58)})
                    </span>
                  </div>
                </div>
              ...
              <div className="p-5 bg-muted/10 font-mono text-[13px] leading-relaxed text-muted-foreground overflow-x-auto whitespace-pre-wrap">
                {input.replace(/please/gi, "").replace(/could you/gi, "").replace(/I want you to/gi, "").slice(0, Math.floor(input.length * 0.8))}
                {"\n\n(This is a simulated optimized result showing a compressed version of your input.)"}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
